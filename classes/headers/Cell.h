...
class Cell {
  public:
   Cell(std::vector<Coordinate> coordinates_vec, double x_size, double y_size,
        double z_size)
   ...
   [[nodiscard]] const std::vector<Coordinate>& CoordinatesVec()
       const noexcept {
      return coordinates_vec;
   }
   [[nodiscard]] std::vector<Coordinate>& CoordinatesVec() noexcept {
      return coordinates_vec;
   }
